...
mixpanel.init('809120297e40ea8fe3c4c922f7b37066', { debug: true });
...
const getQuestionElement = () => {
  const questionBody = document.querySelector('.introduction-wrap');
...
const stylizeChatGPTIcon = (iconElement, isError) => {
  iconElement.style.backgroundColor = isError
    ? 'hsl(358deg 68% 59%)'
    : 'rgb(16, 163, 127)';
...
const createAnswerElement = (isError) => {
  ...
  answerElement.className = 'answer chat-gpt-answer bg-white';
  answerElement.id = isError ? 'chatGPTError' : 'chatGPTAnswer';
  bodyElement.className = ' d-flex p-3';
...
const populateAnswerText = (chatGPTOutput) => {
  ...
  chatGPTOutput.split(CODE_BLOCK_IDENTIFIER).forEach((textBlock, index) => {
    if (index % 2) {
      ...
      preElement.className = `hljs language-${highlightedCode.language}`;
      preElement.innerHTML = highlightedCode.value;
      ...
      textBlock
        .split(INLINE_CODE_IDENTIFIER)
        .forEach((subTextBlock, subIndex) => {
          if (subIndex % 2) {
            // In-line code
...
const populateNonAnswerText = (isError, text) => {
  const textElement = document.getElementById(
    isError ? 'chatGPTErrorText' : 'chatGPTAnswerText',
  );
...
const insertElement = (elementId, createElement) => {
  ...
  if (!element) {
    const answersContainer = document.querySelector('.answer-area');
    element = createElement();

    if (!answersContainer) {
      const qaContainer = document.querySelector('#answer-question');
      qaContainer.parentNode.insertBefore(element, qaContainer);
      return;
    }
    answersContainer.parentNode.insertBefore(element, answersContainer);
  }
};
...
window.onload = () => {
  ...
  populateNonAnswerText(isError, '等待 ChatGPT 响应中...');
  scrapeQuestion();
};
